    ...
    if (dictionaryID.equals("") || openlibraryID.equals("")) {
      ...
      try {
        ...
        for (var part : parts) {
          ...
          if (part.matches(".*\\d.*") || part.matches(".*_.*")) continue;
          if (temp > 5) {
            SetOfWords.add(part);
        ...
        try {
          ...
          if ((double) WordsGreaterEqual9/SetOfWordsSize < 0.2) throw new UnbalancedException("At least 20% of words must have length >= 9");
